...
Opzioni:
  --edition pro|msp       edizione (default: pro)
...
let feat = Object.assign({}, DEFAULT_FEAT[edition] || DEFAULT_FEAT.pro);
if (arg('feat')) {
...
const payload = {
  ...
  iss: today,
  exp: arg('expires', defExp),
  grace: parseInt(arg('grace', '14'), 10),
};
...
console.error('Limiti:     caselle ' + payload.lim.mailboxes + ' | aziende ' + payload.lim.clients + ' | rivenditori ' + payload.lim.resellers);
console.error('Funzioni:   ' + Object.keys(feat).filter((k) => feat[k]).join(', '));
console.error('Scadenza:   ' + payload.exp + ' (grace ' + payload.grace + ' gg)');
